...
int Game::GameInit() {
	...
	r = InitDirect3DDevice(this->hWndMain, 1920, 1080, FALSE, D3DFMT_X8R8G8B8, this->pD3D, &this->pDevice);
	...
	// Load in the objects
	if (FAILED(D3DXLoadMeshFromX(_T("car.x"), D3DXMESH_SYSTEMMEM,
		pDevice, NULL,
		...
		&pMesh2)))
	{
		...
		if (FAILED(D3DXLoadMeshFromX(_T("..\\car.x"), D3DXMESH_SYSTEMMEM,
			...
			&pMesh2)))
		{
			MessageBox(NULL, _T("Could not find Car.x"), _T("Meshes.exe"), MB_OK);
			return E_FAIL;
		}
	}
	...
	for (DWORD i = 0; i<dwNumMaterials2; i++)
	{
			...
			lstrlenA(d3dxMaterials2[i].pTextureFilename) > 0)
		{
			// Create the texture
			if (FAILED(D3DXCreateTextureFromFile(pDevice,
				(CA2CT)d3dxMaterials2[i].pTextureFilename,
				&pMeshTextures2[i])))
			{
				// If texture is not in current folder, try parent folder
				const TCHAR* strPrefix = TEXT("..\\");
				...
				TCHAR strTexture[MAX_PATH];
				lstrcpyn(strTexture, strPrefix, MAX_PATH);
				lstrcpyn(strTexture + lenPrefix, (CA2CT)d3dxMaterials2[i].pTextureFilename, MAX_PATH - lenPrefix);
				// If texture is not in current folder, try parent folder
				if (FAILED(D3DXCreateTextureFromFile(pDevice,
					strTexture,
					&pMeshTextures2[i])))
...
@param x number of points of translation
@param y number of points of translation
@param z number of points of translation
*/
...
@param r number of points of translation
*/
...
@param objNum 0 - camera; 1 - object 1; 2 - object 2
@param x number of points of translation
@param y number of points of translation
@param z number of points of translation
*/
...
/*
 * Get Method to return if the camera is selected.
*/
bool Game::getCameraMove() {
	return CameraMove;
}
/*
 * Get Method to return if object 1 is selected.
*/
bool Game::getObj1Move() {
	return Obj1Move;
}
/*
* Get Method to return if object 2 is selected.
*/
bool Game::getObj2Move() {
	return Obj2Move;
}
/*
* Boolean funtion to set camera move to true(selected) or false(deselected)
*/
...
/*
* Boolean funtion to set object 1 move to true(selected) or false(deselected)
*/
...
/*
* Boolean funtion to set object 2 move to true(selected) or false(deselected)
*/
...
/*
 * Method to set the lighting to directional.
 */
...
/*
 * Method to sets the lighting to point.
*/
...
/*
* Method to turn lighting off.
*/
void Game::SetLightingOff() {
	ZeroMemory(&light, sizeof(light));
	light.Type = D3DLIGHT_POINT;
